# Character Manager

This is a wep-app project for creating and using D&D 5e (2024) characters
Made by Danilo Diedov and Maxim Gorlov as a university coursework.

TODO: [done] creating attacks on char creation
TODO: [done] make item deleting from inventory
TODO: [rejected] create classic sheet as separate db table (maybe?).
TODO: [rejected] export classic sheet to PDF
TODO: [done_bards] test spell selection for wizards and bards
TODO: [done_backend] long rest button to recover hp, spell slots and exhaustion (nothing else probably)
TODO: [done_buggy] level up functionality, probably as separate page
TODO: [done] check if char feats are loaded correctly
TODO: [] add feat selection and adding/deleting them to char
TODO: [] add abililty score editing
TODO: check domain spells